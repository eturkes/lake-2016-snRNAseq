...
```{r}
packages <- c(
  "conflicted", "DT", "SingleCellExperiment", "scater",
  "ggplot2", "BiocFileCache", "biomaRt", "dplyr")
...
We identify and add spike-ins to the SCE object.
...
We add annotations based on the hg38 reference genome, the same used in @lake_neuronal_2016.
...
if (file.exists(rds)) {
  ...
  tmp <- useEnsembl(biomart = "ensembl", dataset = "hsapiens_gene_ensembl")
  attr_string <- c(
  ...
  gene_anno <- getBM(
    attributes = attr_string, filters = "external_gene_name", values = rownames(sce), mart = tmp)
