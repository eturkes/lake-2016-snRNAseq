...
```{r}
counts <- read.delim(counts, "\t", header = FALSE, stringsAsFactors = FALSE)
...
keep_cells <- cell_names %in% anno[ , 2]
...
lake_annoer_name <- as.character(unlist(anno[3]))
...
sce <- SingleCellExperiment(
  assays = list(normcounts = as.matrix(sapply(counts, as.numeric))), colData = anno,
  rowData = gene_names)
rm(counts, anno)
sce
```
...
Take the log2 transformation of the transcript counts.
...
```{r}
# plotColData(sce, aes_string(x = "total_counts", y = "total_features", colour = ct))
```
